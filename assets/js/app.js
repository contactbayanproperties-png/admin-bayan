// Fallback listings data (used if fetch fails)
const FALLBACK_LISTINGS = [
  {
    ...
    highlights: ["Résidence sécurisée", "Parking", "Proche commodités"],
  },
  {
    id: "BP-002",
    ...
    highlights: ["Jardin", "Résidence sécurisée", "Quartier recherché"],
  },
  {
    id: "BP-003",
    ...
    highlights: ["Résidence verte", "Accès rapide", "Bon rapport qualité/prix"],
  },
  {
    id: "BP-004",
  ...
  },
  {
    ...
    highlights: ["Prêt à vivre", "Wifi inclus", "Idéal mobilité"],
  },
  {
    id: "BP-006",
...
// Global listings array - will be populated on load
let PROPERTIES = [];

// Load listings from JSON (supports both array and {items:[...]} format)
async function loadListings() {
  try {
    ...
    return items.length ? items : FALLBACK_LISTINGS;
  } catch (e) {
...
// Normalize CMS data to display format
function normalizeProperty(item) {
  // Map zone to lowercase key for filters
  const zoneMap = {
  ...
  // Map type to lowercase key for filters
  const typeMap = {
    Appartement: "appartement",
    Villa: "villa",
    Studio: "appartement", // Studios shown as appartement for filter
    Terrain: "terrain",
    Bureau: "bureau",
  };
  ...
  return {
    id: item.id,
    title: item.title,
    operation: (item.operation || "Location").toLowerCase(),
    type: typeMap[item.type] || item.type?.toLowerCase() || "appartement",
    zone:
      zoneMap[item.zone] ||
      item.zone?.toLowerCase().replace(" ", "-") ||
      "casablanca",
    price: item.price_mad || item.price || 0,
    currency: "MAD",
    period: item.operation?.toLowerCase() === "location" ? "/mois" : "",
    area: item.surface_m2 || item.area || 0,
    beds: item.bedrooms || item.beds || 0,
    baths: item.bathrooms || item.baths || 0,
    meuble: item.meuble || false,
    charges: item.charges_incluses || item.charges || false,
    dispo: item.dispo !== undefined ? item.dispo : true,
    image:
    ...
    chargesDetails: item.chargesDetails || {},
  };
}
...
function badgeHTML(p) {
  ...
  b.push(
    p.meuble
      ? `<span class="badge badge-meuble">MEUBLÉ</span>`
      : `<span class="badge badge-nonmeuble">NON MEUBLÉ</span>`,
  );
  b.push(
    p.charges
...
function renderListings(targetId, limit = null, filter = null) {
  ...
  el.querySelectorAll('[data-wa="1"]').forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      const msg = `Bonjour Bayan Properties, je suis intéressé(e) par le bien Réf: ${p?.id || id}. Pouvez-vous me proposer une visite ? Merci.`;
      openWhatsApp(msg);
    });
  });
}
...
document.addEventListener("DOMContentLoaded", async () => {
  // Load listings from JSON and normalize them
  const rawListings = await loadListings();
  PROPERTIES = rawListings.map(normalizeProperty);

  // Store globally for debugging
  window.__LISTINGS = rawListings;
